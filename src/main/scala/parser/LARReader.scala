...
object LARReader {
  ...
  def apply(s: String): LAR = {
    val id = s.substring(0, 1).toInt
    val respId = s.substring(1, 10)
    val code = s.substring(10, 11).toInt
    val loanId = s.substring(11, 36)
    val loanDate = s.substring(36, 44)
    val loanType = s.substring(44, 45).toInt
    val propertyType = s.substring(45, 46).toInt
    val loanPurpose = s.substring(46, 47).toInt
    val occupancy = s.substring(47, 48).toInt
    val loanAmount = s.substring(48, 53).toInt
    val preapprovals = s.substring(53, 54).toInt
    val actionType = s.substring(54, 55).toInt
